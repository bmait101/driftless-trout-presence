...
# clean up 
rm(df_surveys_raw); rm(df_surveys)
rm(df_efforts_raw); rm(df_efforts)
```
...
# write to file
write_rds(df_fish_raw, here("data", "fish_raw_20210803.rds"))

# load
df_fish_raw <- read_rds(here("data", "fish_raw_20210803.rds"))
```
...
#### how many surveyed wbics in the driftless are also on classified streams:

So we need to link survey sites to streams to see if they are on classified water or not:
