...
```{r read-lines-polys, cache=TRUE, results = 'hide'}
...
```{r count-survs-in-drift, echo=FALSE}
n.surveys <- 
  df_surveys_drift %>% 
...
```{r count-wbics-in-drift-survs, echo=FALSE}
n.wbics.surveyed <- 
  df_surveys_drift %>% 
  distinct(wbic) %>% 
  pull() %>% 
...
```{r count-wbics-in-class-streams, echo=FALSE}
n.wbics.classified <- 
  lines_classed_drift %>% 
  distinct(WBIC) %>% 
  pull() %>% 
...
df_surveys_drift_classed <- here("data", "df_surveys_drift_classed.rds") %>% read_rds()
```

#### count of classificed wbics that have been surveyed:
```{r count-wbics-in-classed-surved, echo=FALSE}
...
#### surveys where species = 'no_fish_captured':
...
Given the target species of a survey, a zero is added for brooks or browns or both:
...
ggplot() + 
  ...
  labs(
    title = "How common are trout in classified Driftless Area streams?",
    color = "Stream Class", 
    fill = "Trout Presence") + 
  theme_void() + 
  theme(
