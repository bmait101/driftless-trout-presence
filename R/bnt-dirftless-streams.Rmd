...
# sureveys sites for those on classified water
...
ggplot() + 
  ...
  geom_sf(data = df_surveys_drift_sites_unclassed, 
          size = 1, alpha = 0.5, shape = 21, fill = "grey") + 
  geom_sf(data = df_surveys_drift_sites_classed, aes(fill = TROUT_CLAS), 
...
n.wbics.bnt.12 <- 
  df_fish_drift_pa_wide %>% 
  select(wbic, TROUT_CLAS, brown_trout) %>% 
  filter(brown_trout == 1, TROUT_CLAS %in% c("CLASS I", "CLASS II")) %>% 
  pull(wbic) %>% unique() %>% length()
...
n.wbics.bkt.12 <- 
  df_fish_drift_pa_wide %>% 
  select(wbic, TROUT_CLAS, brook_trout) %>% 
  filter(brook_trout == 1, TROUT_CLAS %in% c("CLASS I", "CLASS II")) %>% 
  pull(wbic) %>% unique() %>% length()
...
n.wbics.tiger.12 <- 
  df_fish_drift_pa_wide %>% 
  select(wbic, TROUT_CLAS, tiger_trout) %>% 
  filter(tiger_trout == 1, TROUT_CLAS %in% c("CLASS I", "CLASS II")) %>% 
  pull(wbic) %>% unique() %>% length()
...
n.wbics.symp.12 <- 
  df_fish_drift_pa_wide %>% 
  select(wbic, TROUT_CLAS, symp) %>% 
  filter(symp == 1, TROUT_CLAS %in% c("CLASS I", "CLASS II")) %>% 
  pull(wbic) %>% unique() %>% length()
```
...
ggplot() + 
  geom_sf(data = driftless, fill = "white") +
  geom_sf(data = lines_drift, aes(color = TROUT_CLAS), show.legend = "line") +
  geom_sf(data = df_fish_drift_pa %>% filter(n == 1), aes(fill = TROUT_CLAS), 
          show.legend = "point", size = 1, alpha = 0.5, shape = 21) + 
  scale_colour_manual(
    values = c("#53DC4D", "#00C5ff", "#004DA8"), 
    guide = guide_legend(
