...
#### unique surveys and wbics

```{r}
# print uniqe surveys and wbics in driftles surveys data
df_surveys_drift %>% 
  st_drop_geometry() %>% 
  summarise(across(c(survey.seq.no, wbic), ~ length(unique(.x))))

n.surveys <- length(unique(df_surveys_drift$survey.seq.no))
n.wbics.surveyed <- length(unique(df_surveys_drift$wbic))
```
...
n.wbics.classified <- length(unique(lines_drift$WBIC))
```
